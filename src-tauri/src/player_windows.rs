...
const CLASS_NAME: &str = "SurfexVideo";
...
pub fn player_start(
	...
	let pipe = format!(r"\\.\pipe\surfex-mpv-{}", std::process::id());
	...
	let log = std::env::temp_dir().join(format!("surfex-mpv-{}.log", std::process::id()));
	let _ = std::fs::remove_file(&log);
