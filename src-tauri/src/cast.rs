//! Casting — playing what this device holds on another Surfex on the same network.
//!
...
pub async fn cast_share(enable: bool) -> Result<Option<String>, String> {
	...
	tauri::async_runtime::spawn(async move {
		...
		tokio::select! {
			result = server => {
				if let Err(e) = result {
					eprintln!("[surfex] cast mirror stopped: {e:#}");
				}
			}
	...
	tauri::async_runtime::spawn(async move {
		...
		tokio::select! {
			result = axum::serve(listener, router) => {
				if let Err(e) = result {
					eprintln!("[surfex] cast receiver stopped: {e:#}");
				}
			}
			_ = rx => {}
		}
...
async fn identity(State(state): State<Receiving>) -> Json<Identity> {
	Json(Identity { app: "surfex", name: state.name })
}
...
async fn play(State(state): State<Receiving>, Json(mut command): Json<Play>) -> StatusCode {
	...
	match state.app.emit("cast://play", command) {
		Ok(()) => StatusCode::OK,
		Err(e) => {
			eprintln!("[surfex] cast command could not reach the page: {e:#}");
...
async fn stop(State(state): State<Receiving>, Json(command): Json<Stop>) -> StatusCode {
	...
	match state.app.emit("cast://stop", ()) {
		Ok(()) => StatusCode::OK,
		Err(e) => {
			eprintln!("[surfex] cast stop could not reach the page: {e:#}");
			StatusCode::INTERNAL_SERVER_ERROR
		}
	}
}
