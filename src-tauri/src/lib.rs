...
/// Playing what this device holds on another Surfex on the same network.
mod cast;
...
/// why a `surfex://` link works from Firefox. xdg-open runs the value through its
/// own parser instead, keeps the quotes as part of the word, and bails out when
/// `command -v '"/path/to/surfex"'` finds nothing — silently, falling through to
...
/// `/usr/bin/surfex` from the AUR, a Nix store path or a plain `cargo build`
...
fn disk_space(app: tauri::AppHandle, path: Option<String>) -> Result<DiskSpace, String> {
	let dir = match path.as_deref().map(str::trim).filter(|p| !p.is_empty()) {
		...
		None => cache_dir(&app).join("surfex-torrents"),
	};
	#[cfg(unix)]
	{
...
async fn run_torrent_server(
	...
	let session = match Session::new_with_opts(download_dir.clone(), opts(true)).await {
		...
		Err(e) => {
			...
			eprintln!("[surfex] torrent session failed to start ({e:#}) — retrying without DHT");
	...
	for attempt in 0..20 {
		match TcpListener::bind_tcp(addr, Default::default()) {
			...
			Err(e) => {
				...
				eprintln!("[surfex] port {TORRENT_API_ADDR} busy, retrying… ({e})");
...
pub fn run() {
	...
	// A `surfex://` link clicked while the app is already open must reach the
	...
	builder
		...
		.setup(|app| {
			...
			// would not answer surfex:// links at all. Registering at startup
			...
			#[cfg(desktop)]
			{
				...
				if let Err(e) = app.deep_link().register("surfex") {
					eprintln!("[surfex] could not register the surfex:// scheme: {e}");
				}
			...
			#[cfg(desktop)]
			{
				...
				match tray {
					Ok(Ok(_)) => {}
					Ok(Err(e)) => eprintln!("[surfex] no tray icon: {e}"),
					Err(_) => eprintln!("[surfex] no tray icon: no appindicator library here"),
				}
			}
			...
			let download_dir = cache_dir.join("surfex-torrents");
			let session_dir = cache_dir.join("surfex-session");
			std::fs::create_dir_all(&download_dir).ok();
			std::fs::create_dir_all(&session_dir).ok();

			tauri::async_runtime::spawn(async move {
				if let Err(e) = run_torrent_server(download_dir, session_dir).await {
					eprintln!("[surfex] torrent server exited with error: {e:#}");
				}
			});
...
mod tests {
	...
	#[test]
	fn exec_quotes() {
		let entry = "[Desktop Entry]\nExec=\"/opt/surfex/surfex\" %u\nMimeType=x-scheme-handler/surfex;\n";
		assert!(unquote_exec(entry).contains("Exec=/opt/surfex/surfex %u"));
		// Everything else survives, and a second pass changes nothing.
		assert!(unquote_exec(entry).contains("MimeType=x-scheme-handler/surfex;"));
		assert_eq!(unquote_exec(&unquote_exec(entry)), unquote_exec(entry));
		// A path with a space keeps its quotes — see unquote_exec.
		let spaced = "Exec=\"/home/a b/surfex\" %u\n";
