...
class DownloadService : Service() {
  ...
  override fun onCreate() {
    ...
    lock = getSystemService(PowerManager::class.java)
      ?.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "surfex:downloads")
      ?.apply { setReferenceCounted(false) }
  }

  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    if (worker == null) {
      worker = Thread(::loop, "surfex-downloads").apply { isDaemon = true; start() }
    }
  ...
  private fun notification(state: Snapshot): Notification {
    ...
    return NotificationCompat.Builder(this, CHANNEL)
      ...
      .setContentText(
        if (idle) "Surfex is streaming to it — leave this running" else "${state.percent}% · $speed"
      )
