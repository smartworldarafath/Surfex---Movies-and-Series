...
# source sits at the same indentation and was getting Surfex's revision.
sed -e "s|__TMDB_READ_TOKEN__|$TMDB_API|" \
    -e "s|__TAG__|$tag|" \
    -e "s|__COMMIT__|$commit|" \
    "$here/io.github.smartworldarafath.Surfex.yml" > io.github.smartworldarafath.Surfex.yml
...
sed -e "s|<release version=\"[^\"]*\" date=\"[^\"]*\"/>|<release version=\"$version\" date=\"$(git -C "$root" log -1 --format=%ad --date=short "$tag")\"/>|" \
    "$here/io.github.smartworldarafath.Surfex.metainfo.xml" > io.github.smartworldarafath.Surfex.metainfo.xml
appstreamcli validate --explain io.github.smartworldarafath.Surfex.metainfo.xml

cp "$here/io.github.smartworldarafath.Surfex.desktop" "$here/flathub.json" .

echo
echo "✓ $out"
ls -la io.github.smartworldarafath.Surfex.yml io.github.smartworldarafath.Surfex.metainfo.xml \
       io.github.smartworldarafath.Surfex.desktop flathub.json \
       cargo-sources.json node-sources.json package-lock.json

if [ -n "$flathub" ]; then
	cp io.github.smartworldarafath.Surfex.yml io.github.smartworldarafath.Surfex.metainfo.xml \
	   io.github.smartworldarafath.Surfex.desktop flathub.json \
...
echo "  flatpak-builder --force-clean --user --install builddir $out/io.github.smartworldarafath.Surfex.yml"
echo "  flatpak run io.github.smartworldarafath.Surfex"
