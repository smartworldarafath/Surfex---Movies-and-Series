...
const SECRET = new Set([
  `${PREFIX}trakt`, `${PREFIX}tmdbKey`, `${PREFIX}playlists`, `${PREFIX}castCode`, `${PREFIX}castTarget`,
  `${LEGACY_PREFIX}trakt`, `${LEGACY_PREFIX}tmdbKey`, `${LEGACY_PREFIX}playlists`, `${LEGACY_PREFIX}castCode`, `${LEGACY_PREFIX}castTarget`,
])
...
export function makeBackup(store: KeyStore, at = Date.now()): Backup {
  ...
  for (let i = 0; i < store.length; i++) {
    ...
    if ((key?.startsWith(PREFIX) || key?.startsWith(LEGACY_PREFIX)) && !SECRET.has(key))
      keys[key] = store.getItem(key) ?? ''
  }
  return { app: 'surfex', version: 1, at, keys }
}
...
export function readBackup(text: string): Backup {
  ...
  const keys = Object.fromEntries(
    Object.entries(b.keys)
      .map(([k, value]) => {
        // Normalize legacy prefix to current prefix if needed
        const key = k.startsWith(LEGACY_PREFIX) ? PREFIX + k.slice(LEGACY_PREFIX.length) : k
        return [key, value]
      })
  ...
  return { app: 'surfex', version: 1, at: Number(b.at) || 0, keys }
}
