...
;(function () {
  ...
  function environment() {
    ...
    return [
      ...
      ['Storage', attempt(function () {
        localStorage.setItem('surfex.boot', '1')
        localStorage.removeItem('surfex.boot')
        return 'ok'
      })],
  ...
  function verdict() {
    var v = chromeVersion()
    if (v && v < NEEDS) {
      return 'This device\'s Android System WebView is Chrome ' + v + '. Surfex needs '
        + NEEDS + ' or newer. Update "Android System WebView" (and Chrome) from the '
        + 'Play Store, then reopen Surfex.'
    }
    if (boot.missing.length)
      return 'This webview is missing features Surfex is built with: ' + boot.missing.join(', ') + '.'
  ...
  function report() {
    var lines = ['Surfex failed to start', '', verdict(), '']
    var env = environment()
  ...
  function show() {
    ...
    var html = '<h1 style="margin:0 0 12px;font-size:22px;color:#fff">Surfex couldn\'t start</h1>'
    ...
    html += '<p style="margin:20px 0 0;color:#8a8a8a">Please photograph this screen and add it to '
      + 'github.com/smartworldarafath/Surfex---Movies-and-Series/issues</p>'

    panel = document.createElement('div')
    panel.id = 'surfex-boot-error'
