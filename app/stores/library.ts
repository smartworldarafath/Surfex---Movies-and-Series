...
export const useLibraryStore = defineStore('library', () => {
   ...
   * titleKey.
   */
  const media = useLocalStorage<Record<string, Media>>('surfex.media', {})
  /** Keyed by progressKey — one entry per movie and per episode. */
  const progress = useLocalStorage<Record<string, Progress>>('surfex.progress', {})
   ...
   */
  const favourites = useLocalStorage<Record<string, number>>('surfex.favourites', {})
  const watchlist = useLocalStorage<Record<string, number>>('surfex.watchlist', {})
   ...
   * Live TV grid.
   */
  const favouriteChannels = useLocalStorage<Record<string, number>>('surfex.liveFavourites', {})
