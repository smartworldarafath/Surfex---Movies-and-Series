...
export const useSettingsStore = defineStore('settings', () => {
  ...
  // --- Appearance ---
  const theme = useLocalStorage('surfex.theme', 'dark')
  /** The colour the "Your colour" themes are generated from (see theme/palette). */
  const source = useLocalStorage('surfex.themeSource', DEFAULT_SOURCE)
  /**
   ...
   */
  const themeFromArt = useLocalStorage('surfex.themeFromArt', false)
  /**
   ...
   */
  const colourFromPicture = useLocalStorage('surfex.colourFromPicture', false)
  /** Injected as a plain <style> tag, so it outranks everything in a layer. */
  const customCss = useLocalStorage('surfex.customCss', '')
  /** Zoom for the whole interface. 1 = the sizes the app ships with. */
  const uiScale = useLocalStorage('surfex.uiScale', 1)
  /**
   ...
   */
  const reduceEffects = useLocalStorage('surfex.reduceEffects', isTv() ?? false)
   ...
   */
  const sources = useLocalStorage<string[]>('surfex.sources', [])
   ...
   */
  const quality = useLocalStorage('surfex.quality', '')
   ...
   */
  const playlists = useLocalStorage<string[]>('surfex.playlists', [])
   ...
   */
  const tmdbKey = useLocalStorage('surfex.tmdbKey', '')

  // --- Network ---
  // MB/s, 0 meaning "work it out" (see `uploadLimit` in utils/torrents).
  const downLimit = useLocalStorage('surfex.downLimit', 0)
  const upLimit = useLocalStorage('surfex.upLimit', 0)

  /** Android only — no other platform can tell a metered network from a free one. */
  const wifiOnly = useLocalStorage('surfex.wifiOnly', false)
   ...
   */
  const castReceive = useLocalStorage('surfex.castReceive', false)

  /** What this device is listed as by the one casting to it. */
  const castName = useLocalStorage('surfex.castName', '')
   ...
   */
  const castCode = useLocalStorage('surfex.castCode', '')
  ...
  const downloadDir = useLocalStorage('surfex.downloadDir', '')

  // --- Subtitles ---
  // mergeDefaults: a build that adds a property must not read `undefined` out
  // of the copy stored by the build before it.
  const subs = useLocalStorage<SubtitleStyle>('surfex.subStyle', { ...SUBTITLE_DEFAULTS }, { mergeDefaults: true })
   ...
   */
  const autoSubs = useLocalStorage('surfex.autoSubs', true)
  /** ISO 639 code, as the player last chose or the settings page last set. */
  const subLang = useLocalStorage('surfex.subLang', '')
   ...
   */
  const audio = useLocalStorage<AudioSettings>('surfex.audio', { ...AUDIO_DEFAULTS }, { mergeDefaults: true })
   ...
   */
  const audioByTitle = useLocalStorage<Record<string, AudioSettings>>('surfex.audioByTitle', {})
