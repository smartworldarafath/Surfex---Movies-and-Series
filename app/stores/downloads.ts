...
export const useDownloadsStore = defineStore('downloads', () => {
   ...
   */
  const touched = useLocalStorage<Record<string, number>>('surfex.touched', {})
   ...
   * engine still holds.
   */
  const cached = useLocalStorage<Record<string, { hash: string, file: number }>>('surfex.cached', {})
   ...
   */
  const local = useLocalStorage<Record<string, string>>('surfex.local', {})
  ...
  const cap = useLocalStorage('surfex.storageCap', 0)
  ...
  const peakUp = useLocalStorage('surfex.peakUpload', 0)
