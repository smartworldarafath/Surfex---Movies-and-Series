...
export const useUiStore = defineStore('ui', () => {
  const layout = useLocalStorage<Layout>('surfex.layout', 'grid-detail')
  ...
  const cardWidth = useLocalStorage('surfex.cardWidth', 170)
  /** The faces in a Cast row. Its own setting: a headshot is read at a size a poster isn't. */
  const castWidth = useLocalStorage('surfex.castWidth', 140)
  /** Desktop: collapsed icon-only sidebar. */
  const rail = useLocalStorage('surfex.rail', false)
  ...
  const blur = useLocalStorage('surfex.blur', 44)
  const tint = useLocalStorage('surfex.tint', 0.92)

  /** Where the art behind the app comes from, if anywhere. */
  const backdropMode = useLocalStorage<BackdropMode>('surfex.backdropMode', 'art')
  /** The user's own picture, already downscaled — see `pages/settings/appearance/background.vue`. */
  const backdropImage = useLocalStorage('surfex.backdropImage', '')
  /**
   ...
   */
  const backdropFollowsHover = useLocalStorage('surfex.backdropHover', false)
  /**
   ...
   */
  const artOverCustom = useLocalStorage('surfex.backdropArtOverCustom', true)
  /**
