...
Nuxt 4 + Vuetify 4 front end, Tauri 2 (Rust) back end. A movies and series media library over an
...
- **macOS is not a child window, it is libmpv in-process.** The platform embeds
  ...
  film is up — so macOS uses Android's compositing (`behind`, the `surfex-video`
...
- **Casting is the `url` path pointed sideways.** "Play this on the TV" sends a
  *URL*, never a torrent, so the receiving device plays it exactly as it plays a
  ...
  Stopping is its own route (`POST /surfex/stop`, `cast://stop`) rather than a
  ...
  in and never paused started the television from the top. `surfex.castCode` and `surfex.castTarget` are in `backup.ts`'s
...
- `app/utils/backup.ts` carries every `surfex.` localStorage key by design, so a
...
- **Android is the third answer, and it isn't the updater plugin.** No app may
  ...
  thing. That is `installUpdate`/`updateProgress` on the `SurfexScreen` bridge
...
- **A white screen is a bug report with nothing in it.** The bundle is built for
  ...
  Every error also lands on `window.__surfexBoot` for adb and devtools. Keep
...
- **One colour, four files, none of which can import the other three.** Three
  ...
  `utils/ground.ts` writes whatever is actually on screen to `surfex.ground` and
